...
module.exports = async (user, opts) => {
  const { captchaSolver, smsNumberVerifier, browser } = opts;
  ...
  do {
    if (await page.$("#wlspispHipChallengeContainer")) {
      ...
      if (smsNumberVerifier) {
        ...
        do {
          ...
          if (!info || !info.isValid())
            throw new Error(`received invalid sms number "${number}"`);
          ...
          await page.type(
            "#wlspispHipChallengeContainer input[type=text]",
            shortNumber,
            { delay: 15 }
          );
          await delay(200);
          await page.click(
            '#wlspispHipControlButtonsContainer a[title="Send SMS code"]',
            { delay: 28 }
          );

          let error = null;
          await Promise.race([
            page.waitFor("#wlspispHipChallengeContainer input[type=text]", {
              visible: true
            }),
            page
              .waitFor(".alert-error[aria-hidden=false]", { visible: true })
              .then(() =>
                page.$eval(".alert-error[aria-hidden=false]", e => e.innerText)
              )
              .then(e => {
                error = e.trim();
              })
          ...
          if (error) {
            ...
            if (smsNumberVerifier.provider.addNumberToBlacklist) {
              const result = await smsNumberVerifier.provider.addNumberToBlacklist(
                { service, number }
              );
              console.warn(
                "sms adding to blacklist",
                { service, number },
                result
              );
            }
        ...
        try {
          ...
          if (!authCodes || !authCodes.length)
            throw new Error(
              `unable to retrieve auth code ${number} ${service}`
            );
        } catch (err) {
          if (smsNumberVerifier.provider.addNumberToBlacklist) {
            const result = await smsNumberVerifier.provider.addNumberToBlacklist(
        ...
        for (let i = 0; i < authCodes.length; ++i) {
          ...
          await page.type(
            "#wlspispHipSolutionContainer input[type=text]",
            code
          );
          ...
          await Promise.all([
            ...
            Promise.race([
              page.waitForNavigation({ timeout: 60000 }).then(() => {
                waitForNavigation = false;
              }),
              // page.waitFor('.alert.alert-error', { visible: true })
              page
                .waitFor(".alert-error[aria-hidden=false]", {
                  visible: true,
                  timeout: 60000
                })
                .then(() =>
                  page.$eval(
                    ".alert-error[aria-hidden=false]",
                    e => e.innerText
                  )
                )
                .then(e => {
      ...
      if (captchaSolver) {
        do {
          await page.waitFor(
            '#hipTemplateContainer img[aria-label="Visual Challenge"]',
            { visible: true }
          );
          ...
          const result = await captchaSolver.getTaskResult(taskId, {
            ...
            onFailedAttempt: err => {
              console.log(
                `Error getting captcha task result #${
                  err.attemptNumber
                } failed. Retrying ${err.attemptsLeft} times left...`
              );
            }
          });
          console.log(
            `captcha task result: ${JSON.stringify(result, null, 2)}`
          );

          const solution = result && result.solution && result.solution.text;
          await page.type("#hipTemplateContainer input", solution, {
            delay: 40
          });
  ...
  // ---------------------
  try {
    console.log("đang đợi chuyển vào https://outlook.live.com/mail/inbox");
    await page.waitForNavigation();
  } catch (error) {}
  await delay(800);
  const getStartedBtnSelect = 'button[aria-label="Get started"]';
  let content = await page.content();
  const $ = cheerio.load(content);
  const lang = $("html").attr("lang");
  if (lang == "vi") {
    getStartedBtnSelect = 'button[aria-label="Bắt đầu"]';
  }
  if (lang == "en") {
    getStartedBtnSelect = 'button[aria-label="Get started"]';
  }
  await page.waitFor(getStartedBtnSelect, { visible: true });
  await page.waitFor(500);
  await page.click(getStartedBtnSelect);
  await page.waitFor("div.ms-Panel-commands", { visible: true });
